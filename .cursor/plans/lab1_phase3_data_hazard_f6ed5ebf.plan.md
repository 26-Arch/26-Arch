...
overview: 在 Phase 2 基础上实现数据冒险处理：转发（Forwarding）解决 EX/MEM 与 MEM/WB 的 RAW 冒险；阻塞与气泡（Stall & Bubble）解决 Load-Use 冒险。
todos:
  - id: forwarding-unit
    content: 实现 Forwarding Unit：EX 阶段 ALU 输入前增加 Mux，EX/MEM 与 MEM/WB 旁路，EX 优先
    status: pending
  - id: hazard-detection
    content: 实现 Hazard Detection Unit：检测 Load-Use（mem_read_ex && rd 匹配 rs1_id/rs2_id）
    status: pending
  - id: stall-wire
    content: 将 stall 从常量 0 改为 load_use_hazard 信号
    status: pending
  - id: id-ex-bubble
    content: ID_EX 增加 stall 时的 Bubble 分支，清零所有控制信号
    status: pending
  - id: ex-mem-mem-wb-stall
    content: 确认 EX_MEM、MEM_WB 在 stall 时仍正常更新（让 Load 流动）
    status: pending
  - id: test-verify
    content: 运行 make test-lab1，验证 HIT GOOD TRAP 与无寄存器错误
    status: pending
isProject: false
---
...
**关键**：EX 冒险判断必须写在 MEM 冒险之前（`if-else` 顺序），以正确处理双重冒险（如连续三条 `ADD x1, x1, x2`）。
...
**输出**：`stall` 信号，替代当前 `assign stall = 1'b0`。
...
- 当前：`reset` 初始化，`else if (!stall)` 正常更新
- 修改：`else if (stall)` 时写入气泡（所有控制信号置 0），`else` 正常更新
- 气泡需清零的信号：`mem_read_ex`, `mem_write_ex`, `reg_write_ex`, `inst_valid_ex` 等控制信号；数据可保持或置 0（因控制为 0，不会产生副作用）
...
- Forwarding：EX 冒险优先于 MEM 冒险；`rd == 0` 不触发前递
- Stall：`mem_read_ex && rd 匹配 rs1/rs2` 时拉高 stall
- Bubble：stall 时 ID_EX 控制信号全 0
- 运行 `make test-lab1`，确认出现 `HIT GOOD TRAP` 且无寄存器比对错误
